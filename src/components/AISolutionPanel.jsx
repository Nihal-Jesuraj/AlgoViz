...
import { Code, BookOpen, BrainCircuit, Activity } from 'lucide-react';
    ...
    <div className="w-full h-full flex flex-col bg-[var(--color-surface)] border-r border-[var(--glass-border)] backdrop-blur-md">
      ...
      <div className="p-4 border-b border-[var(--glass-border)] bg-gradient-to-b from-[var(--glass-fill)] to-transparent">
        ...
        <div className="flex gap-2 text-[10px] font-mono">
          <div className="px-2 py-1 bg-[var(--glass-fill)] rounded border border-[var(--glass-border)] text-[var(--color-text-muted)]">
            Algo: <span className="text-accent-purple font-semibold">{customProblemData.algorithmType}</span>
          </div>
          <div className="px-2 py-1 bg-[var(--glass-fill)] rounded border border-[var(--glass-border)] text-[var(--color-text-muted)]">
            Time: <span className="text-accent-teal font-semibold">{customProblemData.timeComplexity || 'O(?)'}</span>
          </div>
          <div className="px-2 py-1 bg-[var(--glass-fill)] rounded border border-[var(--glass-border)] text-[var(--color-text-muted)]">
      ...
      <div className="flex border-b border-[var(--glass-border)] px-2 pt-2 gap-1 bg-[var(--color-surface-sunken)]">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-t-lg font-heading text-xs tracking-wide transition-colors ${
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'explanation' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className={`absolute inset-0 p-5 prose prose-sm max-w-none prose-headings:text-[var(--color-text)] prose-a:text-accent-purple text-[var(--color-text-muted)] ${themeId === 'light' ? '' : 'prose-invert'}`}
            >
