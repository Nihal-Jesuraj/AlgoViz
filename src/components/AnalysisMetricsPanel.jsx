  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-4 mt-6 pb-20 px-6 font-mono">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {cards.map((c) => (
          <div key={c.label} className="glass-panel p-3 rounded-xl bg-white/5 border border-white/10 shadow-sm">
            <div className="text-[10px] text-[var(--color-text-muted)] font-bold tracking-widest uppercase mb-1">{c.label}</div>
            <div className={`text-sm font-extrabold ${c.color} dark:${c.darkColor} truncate`} title={c.value}>{c.value || '--'}</div>
          </div>
        ))}
      </div>

      {/* Correctness Caveat Block */}
      <div className={`p-4 rounded-xl border ${isCorrect ? 'bg-green-500/10 border-green-500/20' : 'bg-orange-500/10 border-orange-500/20'}`}>
        ...
        <div className="mt-2 pt-2 border-t border-black/5 dark:border-white/5 text-[10px] text-[var(--color-text-subtle)] italic">
      ...
      {codeLines && codeLines.length > 0 && (
        <div className="glass-panel rounded-xl border border-white/10 overflow-hidden mt-2 bg-white/5">
          <div className="px-4 py-3 bg-black/5 border-b border-white/5 flex items-center gap-2">
            <span className="text-[10px] font-bold text-[var(--color-text-muted)] tracking-wider uppercase">Line-by-line Explanation</span>
          </div>
          <div className="flex flex-col">
            {codeLines.map((item, i) => (
              <div key={i} className={`flex gap-4 p-3 ${i < codeLines.length - 1 ? 'border-b border-white/5' : ''} items-start`}>
                <span className="w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold bg-black/10 text-[var(--color-text-muted)] flex-shrink-0">
                  {i + 1}
                </span>
                <code className="text-[11px] text-[var(--color-text)] bg-black/5 px-2 py-1 rounded max-w-[240px] flex-shrink-0 overflow-x-auto whitespace-pre-wrap word-break-all">
                  {item.line}
                </code>
